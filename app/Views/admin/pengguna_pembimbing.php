        ...
        <div class="section-header">
            ...
            <div class="section-header-breadcrumb">
                ...
                <div class="breadcrumb-item"><a
                        href="<?= base_url() ?>/<?= session()->get('role') ?>/pengguna/pembimbing">Data Master
                        Pengguna</a>
                </div>
                <div class="breadcrumb-item">Pembimbing Magang</div>
            </div>
        </div>
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-row">
                                <div class="form-group col-md-6">
                                    ...
                                    <select onchange="get_sub()" class="w-100 select2 form-control" id="myOpd">

                                    </select>
                                </div>
                                <div class="form-group col-md-6">
                                    <label>Sub Bagian</label>
                                    <select onchange="get_data()" class="w-100 select2 form-control" id="mySub">
...
function ubah($id_ssuser, $nama, $email, $nip, $id_opd, $status = 1) {
    remove_invalid();
...
function ubah_user($id) {
    ...
    if (document.getElementById('nama').value == "" || isEmptyOrSpaces(document.getElementById('nama').value)) {
        ...
        $.ajax({
            url: "<?= base_url() ?>/<?= session()->get('role') ?>/pengguna/update_op",
            type: "POST",
            data: formData,
            contentType: false,
            processData: false,
...
function tambah_user() {
    ...
    if ($("#mySub").val() == null) {
        ...
        $.ajax({
            url: "<?= base_url() ?>/<?= session()->get('role') ?>/pengguna/set_pembimbing",
            type: "POST",
            data: formData,
            contentType: false,
            processData: false,
...
function update_active($status, $id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                url: "<?= base_url() ?>/<?= session()->get('role') ?>/pengguna/update_status",
                type: "POST",
                data: formData,
                contentType: false,
                processData: false,
...
function del($id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                url: "<?= base_url() ?>/<?= session()->get('role') ?>/pengguna/delete_pembimbing",
                type: "POST",
                data: formData,
                contentType: false,
                processData: false,
